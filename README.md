# Memory Hub iOS

Memory Hub 是一个本地优先的个人记忆与生活信息管理应用。产品规则以 `PRODUCT_SPEC.md` 为准，Figma 与设计交接以 `PROGRESS.md` 为准。

## 当前开发基线

- 原生 SwiftUI
- iOS 17+
- Swift 5.9
- 本地版本化 JSON 数据文件，原子写入
- XcodeGen 工程描述：`project.yml`
- 不依赖账号、服务器或第三方运行时

## 在 Mac 上启动

当前仓库是在 Windows 环境建立的，无法在本机运行 Xcode 编译。复制到装有 Xcode 的 Mac 后，可直接打开：

```bash
cd "/path/to/Memory Hub"
open MemoryHub.xcodeproj
```

仓库同时保留 `project.yml`。工程结构变化较大时，可安装 XcodeGen 后用 `xcodegen generate` 重新生成工程。

选择一个 iOS 17 或更高版本的 iPhone 模拟器运行。应用第一次启动时会在 Application Support 下创建 `MemoryHub/memory-hub-v1.json`。

## 自动编译

仓库包含共享 `MemoryHub` Scheme 和 GitHub Actions 工作流 `.github/workflows/ios-build.yml`。推送到 `master`/`main` 或提交 Pull Request 时，会在 GitHub 的 `macos-15` runner 上使用 Xcode 16.4 编译 iOS Simulator 版本；手动触发工作流时还会启动 iPhone 16 模拟器，安装应用并上传浅色、深色首页截图。该检查不需要开发者证书，也不会签名或发布应用。

Windows 本地仍不能执行 Swift/Xcode 编译。GitHub Actions 首次成功只代表工程通过编译；交互、通知、文件面板和视觉效果仍需后续在真实 Xcode 模拟器或设备上验收。

当前状态：提交 `1ba8897` 已在 GitHub Actions 使用 Xcode 16.4、iOS 18.5 Simulator SDK 完成 Debug clean build，第一版源码具备真实 Xcode 编译通过记录。

模拟器视觉冒烟也已通过：手动工作流 `30782363966` 完成 iPhone 16 安装、启动和双主题截图。首页日期不受设备系统语言影响；浅色颗粒会透过系统材质卡片形成柔和色层，深色界面保持低对比层级；底栏使用无文字的原生四图标材质栏。截图只在手动触发时生成，避免普通提交反复消耗约 3–6 分钟的 macOS runner 时间。

## 已完成的第一阶段闭环

- 四栏主导航与深浅语义主题
- 稳定的系统默认分类
- 新增分类
- 分类进入文档列表
- 文档创建后进入空文档
- 文档标题修改与软删除
- 文档列表编辑状态加入/移出首页提醒池
- 首页从提醒池随机抽取最多三篇，打开首页或主动刷新时重抽
- 新增记录草稿自动保存
- 正式记录的编辑草稿不覆盖原文，点击完成后才发布并写入历史版本
- 放弃新增草稿与放弃编辑草稿
- 正式记录软删除
- 设置页切换跟随系统/深色/浅色
- 长按分类进入管理模式，支持排序、重命名、改色、新增和两种含文档删除策略
- 日历事项新增与编辑，标题必填、时间可选
- 日历事项完成、无视、软删除，以及首页同一事项的完成/无视与短暂撤销
- 统一回收站按分类、文档、记录和日历事项展示，支持恢复和二次确认永久删除
- 文档提醒支持今天隐藏、延后最多 24 小时、永不提醒，以及设置页候选池管理
- 周期事项规则支持每日或指定星期、可选结束日期、停止未来生成与回收站恢复
- 冰箱内容、待采购、15 天冰箱历史，以及物品位置和位置变化历史
- 全局搜索、文档归档、Markdown 小标题与长记录折叠
- JSON 本地备份导入/导出、存储概览和本地通知设置
- 浅色首页颗粒主题资源已加入 Asset Catalog

## 下一阶段

1. 在可交互的 Mac、云 Mac 或真实设备上核对手势、键盘、通知权限与文件导入导出面板。
2. 为新增文档、提醒池、日历事项、冰箱和物品补充可重复的 UI 冒烟路径。
3. 导航动效后置到具备可交互设备验收条件时再处理。
