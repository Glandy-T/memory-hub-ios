...
- 登录所有者可从“我的 → 连接 Android 原生版”下载一次性连接文件；独立设备令牌只能读取待确认候选并提交接受/忽略结果，不能读取完整正式数据库
...
当前自动测试为 24 条，并在 430×932、393×873 手机视口与 1200×900 桌面视口完成真实浏览器交互验收；底部页面切换会回到顶部，固定导航预留安全滚动空间，日历新增与重载持久化均已走通。
