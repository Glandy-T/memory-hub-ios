...
| [Apple HIG: App icons](https://developer.apple.com/design/human-interface-guidelines/app-icons) | Simple, recognizable silhouette with platform-managed masking and current layered-material guidance | Use one bold external-memory hub mark, no text or medical metaphor; preserve generous safe area and let the approved pigment field carry the brand | iOS and Android app icon |
| [Android adaptive icons](https://developer.android.com/develop/ui/compose/system/icon_design_adaptive) | Separate foreground/background layers, safe zone and monochrome system variant | Generate a transparent foreground, pigment background and monochrome mark from one reproducible source | Android launcher icon |
| [Google Play icon specifications](https://developer.android.com/distribute/google-play/resources/icon-design-specifications) | Full-bleed square artwork without pre-applied launcher mask | Keep the 1024 source full bleed and let the store or launcher apply its own mask | Store-ready master icon |
